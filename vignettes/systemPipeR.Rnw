...
Apply the enrichment analysis to the DEG sets obtained the above differential expression analysis. Note in the following example the \Rfunarg{FDR} filter is set here to an unreasonably high value, simply because of the small size of the toy data set used in this vignette.
<<eval=FALSE, keep.source=TRUE>>=
loadData("data/GO")
