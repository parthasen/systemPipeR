...
\noindent Execute \Robject{SYSargs} on a single machine without submitting to a queuing system of a compute cluster. This way the input FASTQ files will be processed sequentially. If available, multiple CPU cores can be used for processing each file. The number of CPU cores (here 4) to use for each process is defined in the \Robject{*.param} file. The \Rfunction{cores(args)} returns this value.  
<<eval=FALSE, keep.source=TRUE>>=
bampaths <- runCommandline(args=args)
@

\noindent Alternatively, the computation can be greatly acclerated by processing the files in parallel on several compute nodes of a cluster, where a scheduling/queuing system is used to manage the load balancing. To avoid oversubscription of CPU cores on each compute node, the value from \Rfunction{cores(args)} is passed on to the submission command, here \Rfunction{qsub}. The number or independent parallel \Rfunction{qsub} processes is defined under in the \Rfunarg{Nqsubs} argument. The following example will run 18 processes in parallel using for each 4 CPU cores. If the resources allow to run all 18 processes at the same time then the example submission will occupy in total 72 CPU cores.  
