...
Instructions for obtaining GO annotations from \Rpackage{biomaRt}. Note there are usually various alternative sources from where those annotations can be obtained, including the \Robject{*.db} genome annotation packages from Bioconductor.
<<eval=FALSE, keep.source=TRUE>>=
library("biomaRt")
