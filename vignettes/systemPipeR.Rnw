...
\Rpackage{systemPipeR} provides utilities for building \textit{end-to-end} analysis pipelines with automated report generation for NGS applications such as RNA-Seq, ChIP-Seq, VAR-Seq and many others. An important feature is support for running command-line software, such as NGS aligners, on both single machines or compute clusters. This includes both interactive job submissions or batch submissions to queuing systems of clusters (tested only with Torque). For instance, \Rpackage{systemPipeR} can be used with most command-line aligners such as \Robject{TopHat 2} \citep{Kim2013-vg} and \Robject{Bowtie 2} \citep{Langmead2012-bs}. Efficient handling of complex sample sets and experimental designs is facilitated by well defined sample definitions which improves reproducibility and user-friendliness of many typical analysis workflows in the NGS area. 
