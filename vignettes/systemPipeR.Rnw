...
\begin{document}
\title{\Rpackage{systemPipeR}: utilities for building NGS analysis pipelines}
\author{Thomas Girke \\
...
\Rpackage{systemPipeR} provides utiities for building \textit{end-to-end} analysis pipelines with automated report generation for NGS applications such as RNA-Seq, ChIP-Seq, VAR-Seq and many others. An important feature is support for running command-line software, such as NGS aligners, on both single machines or compute clusters. This includes both interactive job submissions or batch submissions to queuing systems of clusters (tested only with Torque). For instance, \Rpackage{systemPipeR} can be used with most command-line aligners such as \Robject{TopHat 2} \citep{Kim2013-vg} and \Robject{Bowtie 2} \citep{Langmead2012-bs}.
...
Mini sample FASTQ files used by this overview vignette as well as the associated workflow reporting vignettes can be downloaded from \href{http://biocluster.ucr.edu/~tgirke/projects/systemPipeR_test_data.zip}{\textcolor{blue}{here}}. This data set contains 18 paired-end (PE) read sets from \textit{Arabidposis thaliana} published by \citep[][\href{http://www.ncbi.nlm.nih.gov/sra/?term=SRP010938}{\textcolor{blue}{SRA: SRP010938}}]{Howard2013-fq}. To minimize processing time during testing, each FASTQ file has been subsetted to 90,000-100,000 random sampled PE reads that map to the first 100,000 nucleotides of each chromosome of the \textit{A. thalina} genome. The corresponding reference genome sequence (FASTA) and its GFF annotion files (also provided in the downloadable archive) have been truncated accordingly. This way the entire test sample data set is less than 200MB in storage space. A PE read set has been chosen for this test data set for flexibility, because it can be used for testing both types of analysis routines requiring either SE (single end) reads or PE reads. 
...
\noindent Subset input targets file as needed. Note: for \Rfunction{qsubRun} the \Robject{targets} file needs to contain absolute paths to FASTQ files in the "FileName' column.
<<eval=FALSE, keep.source=TRUE>>=
