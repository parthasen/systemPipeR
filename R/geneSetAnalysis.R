...
readGOorg <- function(myfile, outdir, colno, org) {
	...
	if(org == "Arabidopsis") {
        ...
        write.table(GO_MF_DF, file=paste0(outdir, "/GO_MF_DF"), quote=T, sep="\t")
        ...
        write.table(GO_BP_DF, file=paste0(outdir, "/GO_BP_DF"), quote=T, sep="\t")
        ...
        write.table(GO_CC_DF, file=paste0(outdir, "/GO_CC_DF"), quote=T, sep="\t")
	...
	go_df <- na.omit(go_df)
	write.table(go_df, file=paste0(outdir, "/go_df"), quote=TRUE, sep="\t")
...
sampleDFgene2GO <- function(lib, outdir) {
	...
	mylibbase <- gsub(".db", "", lib) 
        ...
        write.table(GO_MF_DF, file=paste0(outdir, "/GO_MF_DF"), quote=F, sep="\t")
        cat("\n", "Object 'GO_MF_DF' created containing assigned gene-to-MFGO mappings. To use custom mappings, generate data frame with the same structure in col 1-2.", "\n")
        ...
        write.table(GO_BP_DF, file=paste0(outdir, "/GO_BP_DF"), quote=F, sep="\t")
        ...
        write.table(GO_CC_DF, file=paste0(outdir, "/GO_CC_DF"), quote=F, sep="\t")
...
gene2GOlist <- function(outdir, rootUK=FALSE) { # If the argument 'rootUK' is set to TRUE then the root nodes are treated as terminal nodes to account for the new unknown terms 
        ## Import required data frames
	GO_MF_DF <- read.table(file=paste0(outdir, "/GO_MF_DF"), header=T, colClasses = "character")
        GO_BP_DF <- read.table(file=paste0(outdir, "/GO_BP_DF"), header=T, colClasses = "character")
        GO_CC_DF <- read.table(file=paste0(outdir, "/GO_CC_DF"), header=T, colClasses = "character")
...
loadData <- function(outdir) {
        	...
        	if(length(need_affy2gene[need_affy2gene=="affy2locusDF"]) > 0) {
                ...
                GO_BP_DF <<- read.table(file=paste0(outdir, "/GO_BP_DF"), header=T, colClasses = "character")
                GO_CC_DF <<- read.table(file=paste0(outdir, "/GO_CC_DF"), header=T, colClasses = "character")
                if(any(dir(outdir) %in% "go_df")) { go_df <<- read.table(file=paste0(outdir, "/go_df"), header=T, colClasses = "character") }
		load(file=paste0(outdir, "/MF_node_affy_list")); MF_node_affy_list <<- MF_node_affy_list
		load(file=paste0(outdir, "/BP_node_affy_list")); BP_node_affy_list <<- BP_node_affy_list
		load(file=paste0(outdir, "/CC_node_affy_list")); CC_node_affy_list <<- CC_node_affy_list
}
...
## BatchResult <- GOCluster_Report(setlist=DEGlist, method="all", id_type="gene", CLSZ=10, cutoff=0.001, gocats=c("MF", "BP", "CC"), recordSpecGO=c("GO:0003674", "GO:0008150", "GO:0005575"))
